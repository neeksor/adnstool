...
void of_limit(const struct optioninfo *oi, const char *arg, const char *arg2) {
  ov_concurrent_limit= atoi(arg);
  if (ov_concurrent_limit < 0) {
	  ov_concurrent_limit= 0;
  }
}
...
int main(int argc, const char *const *argv) {
  ...
  for (;;) {
    for (;;) {
      qu= ov_asynch ? 0 : processing.head ? processing.head->qu : 0;
