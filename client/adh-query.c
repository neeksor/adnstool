...
int ov_concurrent_limit = 0;

adns_state ads;
struct pending_list outstanding;
struct processing_list processing;
...
static void prep_pending(struct pending_node **pen_r) {
  struct pending_node *pen;
  
  if (ov_pipe && !ads) usageerr("-f/--pipe not consistent with domains on command line");
  ensure_adns_init();
  
  pen= xmalloc(sizeof(*pen));
  LINK_INIT(*pen);
  pen->owner = NULL;
  *pen_r= pen;
}

static void free_pending(struct pending_node* pen) {
  if (pen) free(pen->owner);
  free(pen);
}
...
static void process_pending(void) {
  ...
  while( ov_concurrent_limit == 0 || processing.size < ov_concurrent_limit) {
    if (!outstanding.head) break;
    pen = outstanding.head;
    LIST_UNLINK(outstanding,pen);
    prep_query(&qun,&quflags);
    qun->owner = xstrsave(pen->owner);
    r= adns_submit(ads, qun->owner,
		   ov_type == adns_r_none ? adns_r_addr : ov_type,
		   quflags,
		   qun,
		   &qun->qu);
    if (r) sysfail("adns_submit",r);
    LIST_LINK_TAIL(processing,qun);
    processing.size++;
    free_pending(pen);
  }
}
...
void query_do(const char *domain) {
  ...
  prep_pending(&pen);
  pen->owner= xstrsave(domain);
  LIST_LINK_TAIL(outstanding,pen);
  process_pending();
}

static void dequeue_query(struct query_node *qun) {
  LIST_UNLINK(processing,qun);
  processing.size--;
  free(qun->id);
  free(qun->owner);
  free(qun);
